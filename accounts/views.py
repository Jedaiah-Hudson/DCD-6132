...
OUTLOOK_STATE_SALT = 'outlook-oauth-state'
...
def _get_outlook_oauth_config():
    config = settings.MSAL_CONFIG
    missing_fields = [
        field for field in ('client_id', 'client_secret', 'authority', 'redirect_uri')
        if not config.get(field)
    ]
    if missing_fields:
        raise ValueError(f"Missing Outlook OAuth setting(s): {', '.join(missing_fields)}")
    return config


def _make_outlook_oauth_state(user):
    signer = signing.TimestampSigner(salt=OUTLOOK_STATE_SALT)
    return signer.sign(str(user.id))


def _get_user_from_outlook_oauth_state(state):
    signer = signing.TimestampSigner(salt=OUTLOOK_STATE_SALT)
    user_id = signer.unsign(state, max_age=600)
    return User.objects.get(id=user_id)


def _profile_oauth_redirect(provider, outcome, message='', account_id=None):
    params = {
        provider: outcome,
    }
    if message:
...
def gmail_callback(request):
    ...
    if oauth_error:
        return _profile_oauth_redirect('gmail', 'error', oauth_error)

    if not code or not state:
        return _profile_oauth_redirect('gmail', 'error', 'Missing authorization code or state.')

    try:
        user = _get_user_from_gmail_oauth_state(state)
        config = _get_gmail_oauth_config()
    except (signing.BadSignature, signing.SignatureExpired, User.DoesNotExist):
        return _profile_oauth_redirect('gmail', 'error', 'Invalid or expired OAuth state.')
    except ValueError as exc:
        return _profile_oauth_redirect('gmail', 'error', str(exc))
    ...
    if not access_token:
        return _profile_oauth_redirect('gmail', 'error', 'Google did not return an access token.')
    ...
    if profile_response.status_code != 200:
        return _profile_oauth_redirect('gmail', 'error', 'Failed to fetch Gmail mailbox profile.')

    gmail_address = (profile_response.json().get('emailAddress') or '').strip().lower()
    if not gmail_address:
        return _profile_oauth_redirect('gmail', 'error', 'Google did not return a Gmail address.')
    ...
    if not saved_refresh_token:
        return _profile_oauth_redirect(
            'gmail',
            'error',
    ...
    return _profile_oauth_redirect('gmail', 'connected', account_id=account.id)
...
def outlook_auth(request):
    ...
    try:
        config = _get_outlook_oauth_config()
    except ValueError as exc:
        return Response({'error': str(exc)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    auth_url = client.get_authorization_request_url(
        config['scope'],
        redirect_uri=config['redirect_uri'],
        state=_make_outlook_oauth_state(request.user),
    )
...
def outlook_callback(request):
    ...
    try:
        user = _get_user_from_outlook_oauth_state(state)
        config = _get_outlook_oauth_config()
    except (signing.BadSignature, signing.SignatureExpired, User.DoesNotExist):
    ...
    user_email = (user_response.json().get('mail') or user_response.json().get('userPrincipalName') or '').strip().lower()
    if not user_email:
        return JsonResponse({'error': 'Microsoft did not return a mailbox address.'}, status=400)
    ...
    return redirect(f"{settings.FRONTEND_BASE_URL}/profile?outlook=connected&account={account.id}")
